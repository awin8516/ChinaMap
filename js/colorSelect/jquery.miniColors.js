	...
	$.extend($.fn, {
		...
		miniColors: function(o, data) {
			...
			var create = function(input, o, data) {
				...
				// Create trigger
				var ColorSeriesClass = "";
				if(input.hasClass("ColorSeries")){ColorSeriesClass = "ColorSeries"}
				
				var trigger = $('<a class="miniColors-trigger '+ColorSeriesClass+'" style="background-color: #' + color + '" href="#"></a>');
			...
			var setColor = function(input, hsb, updateInputValue) {
				...
				//alert(input.data('trigger').attr("class"));				
				if(input.data('trigger').hasClass("ColorSeries")){
					var selecthsb = hex2hsb(hex);
					var colorSub = $(".miniColors").not(".ColorSeries,.nameColorValue");
					var step = input.data('trigger').siblings(".colorstep").val();
					
					colorArr = createSeries(selecthsb,colorSub.size(),step);
					
					for(var i=0,k=colorSub.size()-1;i<colorSub.size();i++,k--){								
						colorSub.eq(i).val(colorArr[colorPx==1 || colorPx==2 ? k:i]);
						colorSub.eq(i).next().css("background-color",colorArr[colorPx==1 || colorPx==2 ? k:i]);								
					}
			...
			var createSeries = function(selecthsb,num,step) {
				var hsbArr = [];
				var sPx,bPx;
					if(selecthsb.s < 50 && selecthsb.b > 50){
						colorPx = 1;
						sPx = step;
						bPx = -step;
					}else if(selecthsb.s > 50 && selecthsb.b > 50){
						colorPx = 2;
						sPx = -step;
						bPx = -step;
					}else if(selecthsb.s > 50 && selecthsb.b < 50){
				...
				for(var i=0;i<num;i++){
					hsbArr[i] = { h: selecthsb.h, s: selecthsb.s+i*sPx, b: selecthsb.b+i*bPx };					
					colorArr[i] = "#"+hsb2hex(hsbArr[i]);
				}				
				return colorArr;
			};
